...
from megatron.optimizers import SophiaG
from megatron.utils import (
    OverflowMonitor,
...
from megatron.evals import run_eval_harness
...
def mup_coord_check(neox_args, timers, lr_scheduler, train_data_iterator):
    ...
    def lazy_model(hidden_size):
        def gen():
            ...
            model, _, optimizer, _ = setup_model_and_optimizer(
...
def get_optimizer(model, neox_args):
    ...
    elif neox_args.optimizer_type.lower() == "sophia":
        optimizer = SophiaG(
            param_groups,
            weight_decay=neox_args.weight_decay,
    ...
    elif neox_args.optimizer_type.lower() == "ADAGRAD".lower():
...
def get_learning_rate_scheduler(optimizer, neox_args):
    ...
    if neox_args.wd_free_lr is not None:
        from megatron.schedulers import MultiLR
        ...
        lambda1 = lambda opt: partial_scheduler(opt, start_lr=neox_args.lr)
        lambda2 = lambda opt: partial_scheduler(opt, start_lr=neox_args.wd_free_lr)
...
def backward_step(neox_args, timers, optimizer, model, loss, do_allreduce=False):
    """Backward step."""

    if neox_args.pipe_parallel_size > 0:
        return backward_step_pipe_but_no_pipe(neox_args, timers, optimizer, model, loss, do_allreduce=do_allreduce)
...
def backward_step_pipe_but_no_pipe(neox_args, timers, optimizer, model, loss, do_allreduce=False):

    self = model # deepspeed engine
