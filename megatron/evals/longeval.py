...
import numpy as np
import transformers
from megatron.evals.load import load_checkpoint
from megatron.evals.generation import generate
...
def test_topics_one_sample(model, tokenizer, test_case, output_file, idx, args):
    ...
    if "hhyena-1.5b-8k" in args.model_name_or_path:
        ...
        output = model.generate(
            input.input_ids.to(model.device), max_new_tokens=50, use_cache=use_cache
        )[0]
...
def test_lines_one_sample(model, tokenizer, test_case, output_file, idx, args):
    ...
    if "hhyena-1.5b-8k" in args.model_name_or_path:
        ...
        output = model.generate(
            input.input_ids.to(model.device), max_new_tokens=100, use_cache=use_cache
        )[0]
        output = output[prompt_length:]
...
def longeval_test(model, tokenizer, output_dir, args):
    if args.task == "topics":
        for num_topics in [5, 10, 15, 20, 25]:
            print(
                f"************ Start testing {num_topics} topics per prompt ***********"
            )
            avg_length = 0

            test_file = os.path.join(
                args.test_dir, f"topics/testcases/{num_topics}_topics.jsonl"
            )
        ...
        for num_lines in [200, 300, 400, 500, 600, 680]:
            print(
                f"************ Start testing {num_lines} lines per LRT prompt ************"
            )
            test_file = os.path.join(
                args.test_dir, f"lines/testcases/{num_lines}_lines.jsonl"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-name-or-path", type=str, required=True, help="model path"
    )
    parser.add_argument(
        "--task",
        type=str,
    ...
    parser.add_argument(
        "--test_dir", type=str, default="evaluation", help="Directory of the testcases"
    )
