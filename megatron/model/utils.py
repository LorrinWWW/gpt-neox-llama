...
from megatron.model.norms import LayerNorm, RMSNorm, FlashRMSNorm, ScaleNorm
...
def get_params_for_weight_decay_optimization(module, neox_args):
    ...
    neox_args.wd_free_lr = neox_args.optimizer["params"].get("wd_free_lr")

    lr = (
        neox_args.wd_free_lr
        if getattr(neox_args, "wd_free_lr", None) is not None
        else neox_args.optimizer["params"]["lr"]
    )
    # pop wd_free_lr from optimizer params so it doesn't get passed to the optimizer
    wd_free_lr = neox_args.optimizer["params"].pop("wd_free_lr", None)
    neox_args.wd_free_lr = wd_free_lr
        ...
        if (
            any(
                [
                    ...
                    isinstance(module_, FlashRMSNorm),
                ]
            )
            or (
                neox_args.weight_decay
