...
from transformers.models.gpt_neo.modeling_gpt_neo import (
    GPTNeoSelfAttention as _GPTNeoSelfAttention,
)
...
class GPTNeoSelfAttention(_GPTNeoSelfAttention):
    def __init__(self, config, attention_type):
        ...
        if flash_attn_installed:
            self.flash_attn = FlashAttention(
                softmax_scale=self.scaling, attention_dropout=config.attention_dropout
            )
    ...
    def forward(
        ...
        attn_output, _ = self.flash_attn(
            qkv, causal=True
        )  # assuming that these are autoregressive!!
...
class GPTBlock(nn.Module):
    def __init__(self, config, layer_id, *args, use_checkpoint=True, **kargs):
        ...
        def attn_res(
            x: torch.Tensor, prefix_masks=None, attention_mask=None
        ) -> torch.Tensor:
            res = x
            x = self.ln_1(x)
            x = self.attn(x, prefix_masks=prefix_masks, attention_mask=attention_mask)[
                0
            ]
            return x + res
    ...
    def forward(
        self, x: torch.Tensor, prefix_masks=None, mask=None, *args, **kargs
    ) -> torch.Tensor:
        if mask is not None:
            # bool -> float
...
class GPTClassificationHead(nn.Module):
    ...
    def forward(self, hidden_states, input_ids=None, *args, **kargs):
        ...
        self.score(self.ln_f(pooled_hidden_states))
