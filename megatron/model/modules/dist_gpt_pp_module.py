...
from utils.dist_checkpoint_utils import load_state_dict
...
class GPTStageBase(nn.Module):
    def __init__(self, args, config):
        ...
        if hasattr(args, "model_type"):
            if args.model_type == "gpt2":
                from .hf_gpt2_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "gptj":
                from .hf_gptj_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "gptneo":
                from .hf_gptneo_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "flash_gptneo":
                from .hf_flash_gptneo_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "gptneox":
                from .hf_gptneox_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "flash_gptneox":
                from .hf_flash_gptneox_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "opt":
                from .hf_opt_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "flash_opt":
                from .hf_flash_opt_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "llama":
                from .llama_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "flash_llama":
                from .flash_llama_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "bloom":
                from .hf_bloom_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "flash_bloom":
                from .hf_flash_bloom_modules import GPTBlock, GPTEmbeddings, GPTLMHead
            elif args.model_type == "h3":
                from .h3.ssm_seq import GPTBlock, GPTEmbeddings, GPTLMHead
            else:
    ...
    def _create_first_layer(self):
        ...
        if self.load_pretrained_model:
            print("loading embs")
            load_state_dict(layer, torch.load(f"{self.model_name}/pytorch_embs.pt"))
        return layer
    ...
    def _create_last_layer(self):
        ...
        if self.load_pretrained_model:
            ...
            load_state_dict(layer, torch.load(f"{self.model_name}/pytorch_lm_head.pt"))
        return layer
    ...
    def _create_transformer_layer(self, layer_idx=0):
        ...
        if self.load_pretrained_model:
            ...
            load_state_dict(layer, torch.load(f"{self.model_name}/pytorch_{layer_idx}.pt"))
        return layer
